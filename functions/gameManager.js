...
class GameManager {
  ...
  createGame(
    ...
    this.games.set(
      roomCode,
      new Game(roomCode, gameName, hostSocketId, hostName, settings, emitFunction, debugMode),
    );
    return roomCode;
  }
...
class Game {
  ...
  async loadPromptsFromAIIfNeeded() {
    ...
    try {
      const level = this.settings.cefrLevel || 'B1';
      const cacheKey = `prompts_${level}`;
      ...
      if (!Array.isArray(parsed) || parsed.length === 0) {
        console.error('Local AI returned no usable prompts; falling back to defaults.');
      ...
      if (validPrompts.length < 5) {
        console.error('Too few valid prompts from AI; falling back to defaults.');
        this.prompts = this.generateFallbackPrompts();
        return;
      }
  ...
  async simulateBotAnswers() {
    ...
    this.players.forEach((player) => {
      if (player.isBot) {
        ...
        if (prompt) {
          const promise = generateBotAnswer(prompt.text, this.settings.cefrLevel || 'B1').then(
            (answer) => {
      ...
      answers = this.votingPairs.flatMap((pair) => [
        {
          ...
          answer: pair.player1.answer,
          votes: voteCounts.get(pair.player1.id) || 0,
        },
        {
          playerId: pair.player2.id,
          name: pair.player2.name,
          answer: pair.player2.answer,
          votes: voteCounts.get(pair.player2.id) || 0,
        },
      ]);
    } else {
