...
io.on('connection', (socket) => {
  ...
  socket.on(
    'create-game',
    ({ playerName, gameName, maxRounds, votingMode, cefrLevel, debug = false }) => {
      const settings = {};
      ...
      const roomCode = gameManager.createGame(
        gameName,
        socket.id,
        playerName,
        settings,
      ...
      const joinUrl =
        lanIps.length > 0
          ? `http://${lanIps[0]}:${PORT}/player.html?room=${roomCode}`
          : `http://localhost:${PORT}/player.html?room=${roomCode}`;
  ...
  socket.on('start-game', async ({ roomCode }) => {
    ...
    if (game.players.length < (game.debugMode ? 1 : 2)) {
      socket.emit('error', {
        message: `At least ${game.debugMode ? 1 : 2} player(s) are required to start the game.`,
      });
      return;
    }
